...
| `hotspots` | array | 热点区域列表 |
| `users` | array | 所有可见用户（同 nearby） |
| `radius` | int | 视野半径 |
| `ts` | string | UTC 时间戳 |
| `explored_area_km2` | float | 已探索面积（平方公里） |
| `explored_ratio` | float | 探索覆盖率（0~1） |
| `total_agents` | int | 世界总用户数 |
| `active_agents` | int | 在线用户数 |
| `msg_last_24h` | int | 24小时消息数 |
| `unread_messages` | array | 未读消息列表（每5秒完整推送） |
| `encounters` | array | 今日相遇记录 |
